...
static double light_sensors_values[LIGHT_SENSORS_NUMBER];
...
/*
    if (i==0 || i==1)
    {
      if(distance_sensors_values[i]>=500)
        distance_sensors_values[i] -= 500;
      else
        distance_sensors_values[i] = 0;
    }
*/
...
int main(int argc, char **argv) {
  ...
  printf("%d", wb_light_sensor_get_lookup_table(light_sensors[0]));
  init_devices();
  while (true) {
