//Mean shift method, random walk state
...
int main(int argc, char **argv) {
  ...
  printf("Mean shift method controller of the e-puck robot started...\n");
  ...
  while (true) {
    ...
    for (i = 0; i < DISTANCE_SENSORS_NUMBER; i++) {
      if(distance_sensors_values[i] < distance_min){
        distance_min = distance_sensors_values[i];
      }
    }
    
    for (i = 0; i < LIGHT_SENSORS_NUMBER; i++) {
      if(i != 1 && i != 6 ){
        if(light_sensors_values_pure[i] < light_min){
          light_min = light_sensors_values_pure[i];
        }
      }
    }
  
    if (distance_min > 0.1)  //close to an obstacle
    { 
      run_braitenberg();
    } 
    else if(light_min < 1999) //close to light source
    {
      ...
      do{
        ...
        //find the max value of the two front light sensors
        if(light_sensors_values_pure[0] < light_sensors_values_pure[7])
          max_07 = light_sensors_values_pure[7];
        else
          max_07 = light_sensors_values_pure[0];
        
        //find the min value of the other 4 light sensors
        for(i = 2; i < LIGHT_SENSORS_NUMBER-2; i++){
          if(light_sensors_values_pure[i]<min_others)
            min_others = light_sensors_values_pure[i];
        }
        
        //start rotating robots
        turn_left();
        
       }while(max_07 > min_others+200 || abs(light_sensors_values_pure[0]-light_sensors_values_pure[7]) > 200);
